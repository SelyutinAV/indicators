...
2. В поле "Формула" укажите формулу, используя названия показателей в квадратных скобках:
   - `[Показатель1] + [Показатель2]` - простая сумма
   - `[Показатель1] * 100 / [Показатель2]` - расчет процента
   - `([Показатель1] + [Показатель2]) / 2` - среднее значение
3. Для агрегации значений за период используйте функции:
   - `SUM([Выручка], 'month')` - сумма за месяц
   - `AVG([Температура], 'quarter')` - среднее за квартал
   - `MAX([Продажи], 'year')` - максимум за год
   - `MIN([Температура], 'month')` - минимум за месяц
   - `COUNT([Показатель], 'month')` - количество значений за месяц
4. Доступные периоды: `'day'`, `'month'`, `'quarter'`, `'year'`
5. Система автоматически проверит формулу на валидность и циклические зависимости
